...
.word  _ebss
/* start address for the .RamFunc section. defined in linker script */
.word  _siRamFunc
/* start address for the .RamFunc section. defined in linker script */
.word  __ram_func_start
/* end address for the .RamFunc section. defined in linker script */
.word  __ram_func_end
...
/* Copy RamFunc segment initializers from flash to DTCMRAM.
 * Functions marked __attribute__((section(".RamFunc"))) have their load
 * image in flash but must execute from RAM to remain safe during flash
 * erase and program operations. */
  ldr r0, =__ram_func_start
  ldr r1, =__ram_func_end
  ldr r2, =_siRamFunc
  movs r3, #0
  b LoopCopyRamFunc
